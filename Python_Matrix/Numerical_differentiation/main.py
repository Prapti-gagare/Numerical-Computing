...
def run_method(f, method, x, exact_value, h_values, writer):
    ...
    for h in h_values:
        ...
        writer.writerow([f.get_name(), method.get_name(), h, approx, exact_value, error])
...
def main():
    ...
    h_values = [float(input(f"Enter h[{i + 1}]: ")) for i in range(n)]
    ...
    with open("results.csv", "w", newline="") as csv_file:
        ...
        for f in functions:
            ...
            for method in methods:
                run_method(f, method, x, exact_value, h_values, writer)
